...
_git_flow ()
{
    local subcommands="init feature release hotfix change sync files show help version"
...
__git_flow_change ()
{
	local subcommands="list start master production help"
	local subcommand="$(__git_find_on_cmdline "$subcommands")"
	if [ -z "$subcommand" ]; then
		__gitcomp "$subcommands"
		return
	...
	case "$subcommand" in
	master)
		__gitcomp "$(__git_flow_list_branches 'change')"
		return
		;;
	production)
		__gitcomp "$(__git_flow_list_branches 'change')"
...
__git_flow_list_branches2 ()
{
	local prefix="$(__git_flow_prefix $1)"
	git branch 2> /dev/null | tr -d ' |*' | grep "^$prefix" | sort
}
